...
public  class Ebo_Template extends netgest.bo.impl.Ebo_TemplateImpl implements  Serializable {  
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	{
            	brig.add( ret=new BridgeHandlerfromObjectClass(new DataResultSet( getBridgeDataSet("fromObjectClass") ) , this ) );        
        	}
        	if( bridgeName.equals("mappingAttributes") && (ret=brig.get("mappingAttributes"))==null )
        	{
            	brig.add( ret=new BridgeHandlermappingAttributes(new DataResultSet( getBridgeDataSet("mappingAttributes") ) , this ) );        
        	}
        	...
        	{
            	brig.add( ret=new BridgeHandlertemplateOwners(new DataResultSet( getBridgeDataSet("templateOwners") ) , this ) );        
        	}
        	if( bridgeName.equals("KEYS") && (ret=brig.get("KEYS"))==null )
        	{
            	brig.add( ret=new BridgeHandlerKEYS(new DataResultSet( getBridgeDataSet("KEYS") ) , this ) );        
        	}
        	if( bridgeName.equals("KEYS_PERMISSIONS") && (ret=brig.get("KEYS_PERMISSIONS"))==null )
        	{
            	brig.add( ret=new BridgeHandlerKEYS_PERMISSIONS(new DataResultSet( getBridgeDataSet("KEYS_PERMISSIONS") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerKEYS extends bridgeHandler {
		...
		public final class BridgeHanlderKEYSRow extends boBridgeRow
		{
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("securityType")==null)
					xatt.add( new HandlersecurityType( getParent(), this ) );
...
return null;
        ...
        public final class HandlersecurityType extends BridgeAttributeHandler {

            private boolean isdbbinding=true;
            private BigDecimal fieldvalue;
            private boBridgeRow p_bridgerow;

			public HandlersecurityType(boObject parent, boBridgeRow bridgerow ) {
                super( parent , bridgerow ,bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("securityType") );
                p_bridgerow = bridgerow;
            }
            ...
            public BigDecimal getValue() throws boRuntimeException {
                ...
                fireEvent( boEvent.EVENT_BEFORE_GETVALUE, null );
                if(isdbbinding) {
					ret=p_bridgerow.getDataRow().getBigDecimal("SECURITYTYPE");
                } else {
                    ret = fieldvalue;
                }
                fireEvent( boEvent.EVENT_AFTER_GETVALUE, ret );
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                if(getParent().alreadyCalculated(table, dependence)) 
                {                       
                    if(getParent().isWaiting(table, "securityType"))
                    {
                       getParent().setCalculated(table, "securityType");
                       setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "securityType"))
                    {
                        getParent().setCalculated(table, "securityType");
                        setValue(boConvertUtils.convertToBigDecimal(formula(), this), table );
                    }
                ...
                {
                    if(getParent().onChangeSubmit("securityType"))
                    {
                        getParent().setCalculated(table, "securityType"); 
                    }
                    else
                    {
                        getParent().clear(table, "securityType"); 
                        setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    } 
                }
                else
                {    
                    ...
                    if(getParent().onChangeSubmit("securityType"))
                    {
                        getParent().setWaiting(table, "securityType");
                        getParent().calculateFormula(new Hashtable(table), "securityType");
                    }                    
                }
            }
            ...
            {
                if (getParent().isCheckSecurity() && !this.hasRights())
                    	throw new boRuntimeException(HandlersecurityType.class.getName() +
	        	".setValue()", "BO-3230", null, "");                      	                   	                                   	
                if(!ClassUtils.compare(newval,this.getValue())) {
                    BigDecimal chgval = this.getValue();
                    ...
                    if ( fireBeforeChangeEvent( this, chgval, newval) )
                    {
                            if(canAlter(recalc?AttributeHandler.INPUT_FROM_USER:AttributeHandler.INPUT_FROM_INTERNAL))
                            {
                                ...
                                if( recalc && getParent().onChangeSubmit("securityType"))
                                {
                                    Hashtable table = new Hashtable();
                                    getParent().setCalculated(table, "securityType");
                                    getParent().calculateFormula(table, "securityType");
                                }
	                        
                                //vou verificar se o objecto se encontra numa bridge
                                if ( getParent().p_parentBridgeRow != null )
                                {
                                    getParent().getParentBridgeRow().getBridge().lineChanged("securityType");
                                }
            ...
            public void _setValue(BigDecimal newval) throws boRuntimeException {
                if(isdbbinding) {
                        ...
                        p_bridgerow.getDataRow().updateBigDecimal("SECURITYTYPE",newval);
                        getParent().setChanged(true);
                        super.setValid();
