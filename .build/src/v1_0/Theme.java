...
public  class Theme extends boObject implements  Serializable {  
    ...
    public netgest.bo.runtime.attributes.boAttributeString defaultTheme;    
    ...
    public Theme() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("Theme");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("files"))); 
        ...
        atts.add(defaultTheme = new netgest.bo.runtime.attributes.boAttributeString(this,"defaultTheme"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( defaultTheme.haveDefaultValue()) 
                {
                	if(defaultTheme.defaultValue() != null)
                	{
                    	defaultTheme._setValue(boConvertUtils.convertToString(defaultTheme.defaultValue(), defaultTheme));
                    }
                    else
                    {
                    	defaultTheme._setValue(null);
                    }
                    defaultTheme.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("files") && (ret=brig.get("files"))==null )
        	{
            	brig.add( ret=new BridgeHandlerfiles(new DataResultSet( getBridgeDataSet("files") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerfiles extends bridgeHandler {
        ...
        public BridgeHandlerfiles(DataResultSet data,boObject parent) {
            super("files",
                  data,
                  parent.getBoDefinition().getAttributeRef("files").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("files").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("files").getBridge().getChildFieldName();
        ...
        public void add(BigDecimal boui,int xnrow, byte type) throws boRuntimeException {
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("files"))
                  {
                      getParent().calculateFormula("files");
                  }
		...
		{
			return new BridgeHanlderfilesRow( this, row );
		}

		public final class BridgeHanlderfilesRow extends boBridgeRow
		{
			public BridgeHanlderfilesRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("files")==null)
					xatt.add( new Handlerfiles( getParent(), this ) );
...
return null;
        ...
        public final class Handlerfiles extends BridgeObjAttributeHandler {
            public Handlerfiles( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("files")?parent.getBoDefinition().getAttributeRef("files"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("files") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("files"))
                    {
                        ...
                        getParent().setCalculated(table, "files");
                        getParent().calculateFormula(table, "files");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"files"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "files"))
                    {
                       getParent().setCalculated(table, "files");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "files"))
                    {
                        getParent().setCalculated(table, "files");
                ...
                {   
                    ...
                    if(getParent().onChangeSubmit("files"))
                    {                 
                        getParent().setWaiting(table, "files");
                        getParent().calculateFormula(new Hashtable(table), "files");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("files"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "files");
                    getParent().calculateFormula(table, "files");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("files");
				}
            }
...
return new String[] {"files", "name", "description", "defaultTheme"} ;
}
