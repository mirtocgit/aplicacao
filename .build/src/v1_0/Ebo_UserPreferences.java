...
public  class Ebo_UserPreferences extends boObject implements  Serializable {  
    ...
    public Ebo_UserPreferences() {
        super();
        bo_version      = "1.0";
        bo_name         = "Ebo_UserPreferences";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("Ebo_UserPreferences");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("favorites"))); 
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
            ...
            if("Ebo_UserPreferences".equals(this.getName()))
            {
    ...
    public bridgeHandler favorites()
    {
        return getBridge("favorites");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("favorites") && (ret=brig.get("favorites"))==null )
        	{
            	brig.add( ret=new BridgeHandlerfavorites(new DataResultSet( getBridgeDataSet("favorites") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerfavorites extends bridgeHandler {
        ...
        public BridgeHandlerfavorites(DataResultSet data,boObject parent) {
            super("favorites",
                  data,
                  parent.getBoDefinition().getAttributeRef("favorites").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("favorites").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("favorites").getBridge().getChildFieldName();
        ...
        public void add(BigDecimal boui,int xnrow, byte type) throws boRuntimeException {
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("favorites"))
                  {
                      getParent().calculateFormula("favorites");
                  }
		...
		{
			return new BridgeHanlderfavoritesRow( this, row );
		}

		public final class BridgeHanlderfavoritesRow extends boBridgeRow
		{
			public BridgeHanlderfavoritesRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("favorites")==null)
					xatt.add( new Handlerfavorites( getParent(), this ) );
...
return null;
        ...
        public final class Handlerfavorites extends BridgeObjAttributeHandler {
            public Handlerfavorites( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("favorites")?parent.getBoDefinition().getAttributeRef("favorites"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("favorites") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("favorites"))
                    {
                        ...
                        getParent().setCalculated(table, "favorites");
                        getParent().calculateFormula(table, "favorites");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"favorites"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "favorites"))
                    {
                       getParent().setCalculated(table, "favorites");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "favorites"))
                    {
                        getParent().setCalculated(table, "favorites");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "favorites"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("favorites"))
                    {                 
                        getParent().setWaiting(table, "favorites");
                        getParent().calculateFormula(new Hashtable(table), "favorites");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("favorites"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "favorites");
                    getParent().calculateFormula(table, "favorites");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("favorites");
				}
            }
...
return new String[] {"favorites", "owner"} ;
}
