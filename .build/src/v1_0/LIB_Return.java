...
public  class LIB_Return extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeNumber id;    
    public netgest.bo.runtime.attributes.boAttributeString note;    
    ...
    public LIB_Return() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("LIB_Return");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("books"))); 
        ...
        atts.add(note = new netgest.bo.runtime.attributes.boAttributeString(this,"note"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( note.haveDefaultValue()) 
                {
                	if(note.defaultValue() != null)
                	{
                    	note._setValue(boConvertUtils.convertToString(note.defaultValue(), note));
                    }
                    else
                    {
                    	note._setValue(null);
                    }
                    note.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("LIB_Return".equals(this.getName()))
            {
    ...
    public bridgeHandler books()
    {
        return getBridge("books");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("books") && (ret=brig.get("books"))==null )
        	{
            	brig.add( ret=new BridgeHandlerbooks(new DataResultSet( getBridgeDataSet("books") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlerbooks extends bridgeHandler {
        ...
        public BridgeHandlerbooks(DataResultSet data,boObject parent) {
            super("books",
                  data,
                  parent.getBoDefinition().getAttributeRef("books").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("books").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("books").getBridge().getChildFieldName();
        ...
        public void add(BigDecimal boui,int xnrow, byte type) throws boRuntimeException {
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  {
                      getParent().calculateFormula("books");
                  }
		...
		{
			return new BridgeHanlderbooksRow( this, row );
		}

		public final class BridgeHanlderbooksRow extends boBridgeRow
		{
			public BridgeHanlderbooksRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("books")==null)
					xatt.add( new Handlerbooks( getParent(), this ) );
...
return null;
        ...
        public final class Handlerbooks extends BridgeObjAttributeHandler {
            public Handlerbooks( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("books")?parent.getBoDefinition().getAttributeRef("books"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("books") );
            }
