...
public  class LIB_Publisher extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString name;    
    public netgest.bo.runtime.attributes.boAttributeString address;    
    public netgest.bo.runtime.attributes.boAttributeString website;    
    ...
    public LIB_Publisher() {
        super();
        bo_version      = "1.0";
        bo_name         = "LIB_Publisher";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("LIB_Publisher");
        ...
        atts.add(new boBridgeMasterAttribute(this,this.getBoDefinition().getAttributeRef("contacts"))); 
        ...
        atts.add(address = new netgest.bo.runtime.attributes.boAttributeString(this,"address"));
        
        atts.add(website = new netgest.bo.runtime.attributes.boAttributeString(this,"website"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( address.haveDefaultValue()) 
                {
                	if(address.defaultValue() != null)
                	{
                    	address._setValue(boConvertUtils.convertToString(address.defaultValue(), address));
                    }
                    else
                    {
                    	address._setValue(null);
                    }
                    address.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            
                if( website.haveDefaultValue()) 
                {
                	if(website.defaultValue() != null)
                	{
                    	website._setValue(boConvertUtils.convertToString(website.defaultValue(), website));
                    }
                    else
                    {
                    	website._setValue(null);
                    }
                    website.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("LIB_Publisher".equals(this.getName()))
            {
    ...
    public bridgeHandler contacts()
    {
        return getBridge("contacts");        
    }
    ...
    public bridgeHandler getBridge( String bridgeName ) 
    {
        ...
        if( ret == null )
        {
        	...
        	if( bridgeName.equals("contacts") && (ret=brig.get("contacts"))==null )
        	{
            	brig.add( ret=new BridgeHandlercontacts(new DataResultSet( getBridgeDataSet("contacts") ) , this ) );        
        	}
        	if( ret == null )
        	{
    ...
    public final class BridgeHandlercontacts extends bridgeHandler {
        ...
        public BridgeHandlercontacts(DataResultSet data,boObject parent) {
            super("contacts",
                  data,
                  parent.getBoDefinition().getAttributeRef("contacts").getBridge().getChildFieldName(),
                  parent
                 );

            p_fatherfield = parent.getBoDefinition().getAttributeRef("contacts").getBridge().getFatherFieldName();
            p_childfield  = parent.getBoDefinition().getAttributeRef("contacts").getBridge().getChildFieldName();
        ...
        public void add(BigDecimal boui,int xnrow, byte type) throws boRuntimeException {
			  ...
			  if( att.fireBeforeChangeEvent( att, null, newval) ) 
			  {
                  ...
                  if(getParent().onChangeSubmit("contacts"))
                  {
                      getParent().calculateFormula("contacts");
                  }
		...
		{
			return new BridgeHanldercontactsRow( this, row );
		}

		public final class BridgeHanldercontactsRow extends boBridgeRow
		{
			public BridgeHanldercontactsRow( bridgeHandler bridge, netgest.bo.data.DataRow row )
			{
				super( bridge , row );
				addLine();
			}
			...
			public void addLine() 
			{
				...
				if(this.getAttribute("contacts")==null)
					xatt.add( new Handlercontacts( getParent(), this ) );
...
return null;
        ...
        public final class Handlercontacts extends BridgeObjAttributeHandler {
            public Handlercontacts( boObject parent , boBridgeRow bridgerow ) {
                super( parent , bridgerow , bridgerow.getBridge().getDefAttribute().getName().equals("contacts")?parent.getBoDefinition().getAttributeRef("contacts"):bridgerow.getBridge().getDefAttribute().getBridge().getAttributeRef("contacts") );
            }
            public void setValueString(String value) {
                try {
                    ...
                    if(getParent().onChangeSubmit("contacts"))
                    {
                        ...
                        getParent().setCalculated(table, "contacts");
                        getParent().calculateFormula(table, "contacts");
                    }
                } catch (Exception e) {
            ...
            public String getValueString() throws boRuntimeException {
                try {
                    ...
                    String[] args = {getParent().getName(),"contacts"};
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "contacts"))
                    {
                       getParent().setCalculated(table, "contacts");
                       this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "contacts"))
                    {
                        getParent().setCalculated(table, "contacts");
                ...
                else if(getParent().dependsFromWaiting(table, dependence))
                {
                    ...
                    {
                        getParent().clear(table, "contacts"); 
                        this.setValue(boConvertUtils.convertToBigDecimal(formula(), this), false );
                    }
                }
                else
                {   
                    ...
                    if(getParent().onChangeSubmit("contacts"))
                    {                 
                        getParent().setWaiting(table, "contacts");
                        getParent().calculateFormula(new Hashtable(table), "contacts");
                    }                    
                }
            }
            ...
            public void setValue(BigDecimal newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if( recalc && getParent().onChangeSubmit("contacts"))
                {
                    Hashtable table = new Hashtable();
                    getParent().setCalculated(table, "contacts");
                    getParent().calculateFormula(table, "contacts");
				...
				{
					getParent().getParentBridgeRow().getBridge().lineChanged("contacts");
				}
            }
