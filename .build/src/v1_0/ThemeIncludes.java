...
public  class ThemeIncludes extends boObject implements  Serializable {  

   
   
   
    public netgest.bo.runtime.attributes.boAttributeString filePath;    
    ...
    public ThemeIncludes() {
        super();
        bo_version      = "1.0";
        bo_name         = "ThemeIncludes";
        bo_classregboui = "#BO.CLSREGBOUI#";
        bo_definition   = boDefHandler.getBoDefinition("ThemeIncludes");
        ...
        atts.add(filePath = new netgest.bo.runtime.attributes.boAttributeString(this,"filePath"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                ...
                if( filePath.haveDefaultValue()) 
                {
                	if(filePath.defaultValue() != null)
                	{
                    	filePath._setValue(boConvertUtils.convertToString(filePath.defaultValue(), filePath));
                    }
                    else
                    {
                    	filePath._setValue(null);
                    }
                    filePath.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("ThemeIncludes".equals(this.getName()))
            {
...
return new String[] {"filePath", "description", "id"} ;
}
