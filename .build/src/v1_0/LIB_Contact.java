...
public  class LIB_Contact extends boObject implements  Serializable {  

   
   
    public HandlerbusinessCard businessCard;    
   
    public netgest.bo.runtime.attributes.boAttributeString name;    
    public netgest.bo.runtime.attributes.boAttributeString phone;    
    ...
    public LIB_Contact() {
        ...
        bo_definition   = boDefHandler.getBoDefinition("LIB_Contact");
        ...
        atts.add(businessCard = new HandlerbusinessCard(this));
        
        
       
        atts.add(name = new netgest.bo.runtime.attributes.boAttributeString(this,"name"));
        
        atts.add(phone = new netgest.bo.runtime.attributes.boAttributeString(this,"phone"));
    ...
    public void init() throws boRuntimeException {
        ...
        if(p_mode == MODE_NEW && !p_clone)
        {
                    ...
                    if( businessCard.haveDefaultValue()) 
                    {
                    	if(businessCard.defaultValue() != null)
                    	{
                        	businessCard._setValue(boConvertUtils.convertToString(businessCard.defaultValue(), businessCard));
                        }
                        else
                        {
                        	businessCard._setValue(null);
                        }
                        businessCard.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                    }
                ...
                if( phone.haveDefaultValue()) 
                {
                	if(phone.defaultValue() != null)
                	{
                    	phone._setValue(boConvertUtils.convertToString(phone.defaultValue(), phone));
                    }
                    else
                    {
                    	phone._setValue(null);
                    }
                    phone.setInputType( AttributeHandler.INPUT_FROM_DEFAULT );
                }
            ...
            if("LIB_Contact".equals(this.getName()))
            {
    ...
    public final class HandlerbusinessCard extends AttributeHandler {
        private boolean isdbbinding=true;
        private String fieldvalue;
        public HandlerbusinessCard(boObject parent) {
            super(parent,parent.getBoDefinition().getAttributeRef("businessCard"));
        }
        ...
        public String getValueString() throws boRuntimeException {
            ...
            public String getValue() throws boRuntimeException {
                ...
                if (getParent().isCheckSecurity() && !this.hasRights())
	            	throw new boRuntimeException(HandlerbusinessCard.class.getName() +
	        	".getValue()", "BO-3230", null, "");                      	                   	                                 

                fireEvent( boEvent.EVENT_BEFORE_GETVALUE, null );
                if(isdbbinding) {
                    ret = this.getParent().getDataRow().getString("BUSINESSCARD");
                } else {
                    ret = fieldvalue;
                }
            ...
            public void setValueFormula(Hashtable table, String[]dependence) throws boRuntimeException
            {
                ...
                {                       
                    if(getParent().isWaiting(table, "businessCard"))
                    {
                       getParent().setCalculated(table, "businessCard");
                       setValue(boConvertUtils.convertToString(formula(), this), false );
                    }
                    else if(!getParent().isCalculated(table, "businessCard"))
                    {
                        getParent().setCalculated(table, "businessCard");
                ...
                {
                    if(getParent().onChangeSubmit("businessCard"))
                    {
                        getParent().setCalculated(table, "businessCard"); 
                    }
                    else
                    {
                        getParent().clear(table, "businessCard"); 
                        setValue(boConvertUtils.convertToString(formula(), this), false );
                    } 
                }
                else
                {    
                    ...
                    if(getParent().onChangeSubmit("businessCard"))
                    {
                        getParent().setWaiting(table, "businessCard");
                        getParent().calculateFormula(new Hashtable(table), "businessCard");
                        //getParent().setCalculated(table, "businessCard");
                    }                    
                }
            }
            ...
            public void setValue(String newval, boolean recalc) throws boRuntimeException 
            {
                ...
                if(!ClassUtils.compare(newval,this.getValue())) {
                    ...
                    if ( fireBeforeChangeEvent( this, chgval, newval) )
                    {
	                        ...
	                        if( recalc && getParent().onChangeSubmit("businessCard"))
	                        {
	                            Hashtable table = new Hashtable();
	                            getParent().setCalculated(table, "businessCard");
	                            getParent().calculateFormula(table, "businessCard");
	                        }
						    //vou verificar se o objecto se encontra numa bridge
							if ( getParent().p_parentBridgeRow != null )
							{
								getParent().getParentBridgeRow().getBridge().lineChanged("businessCard");
							}
            ...
            public void _setValue(String newval) throws boRuntimeException {
                if(isdbbinding) {
                    this.getParent().getDataRow().updateString("BUSINESSCARD",newval);
